...
$$ F_x = [p - (p + \frac{\partial p}{\partial x}dx)]dy dz \ \ 压力$$
$$ + [(\tau_{xx} + \frac{\partial \tau_{xx}}{\partial x}dx) - \tau_{xx}]dy dz + [(\tau_{yx} + \frac{\partial \tau_{yx}}{\partial y}dy) - \tau_{yx}]dx dz + [(\tau_{zx} + \frac{\partial \tau_{zx}}{\partial z}dz) - \tau_{xx}]dx dy \ \ 应力$$
$$  + \rho f_x (dx dy dz) \ \ 体积力$$
